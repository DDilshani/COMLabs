...
sem_t readLock; 
sem_t writeLock;
sem_t queue; // To get first come first serve access
...
void* reader(void *arg) { 
	...
	while(1) { 
		...
		sem_wait(&queue); // Wait for access in order
		sem_wait(&readLock); 
		sem_post(&queue);	// release the lock for order
		readerCount++;
...
int main(int argc, char **argv) { 
	...
	while(1) { 
		...
		sem_wait(&queue); 	// wait to get access in order
		sem_wait(&writeLock);
		sem_post(&queue); 	//release the lock for order
